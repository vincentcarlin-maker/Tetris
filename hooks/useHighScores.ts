...
export const useHighScores = () => {
  ...
  useEffect(() => {
    try {
      ...
      if (stored) {
        ...
        if (!parsed.neonblocks) parsed.neonblocks = 0;
  ...
  const updateHighScore = useCallback((game: keyof HighScores, value: number, subkey?: string | number) => {
    setHighScores(prev => {
      ...
      if (['tetris', 'breaker', 'pacman', 'snake', 'invaders', 'watersort', 'runner', 'stack', 'arenaclash', 'lumen', 'slither', 'neonblocks'].includes(game)) {
