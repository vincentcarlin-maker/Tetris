...
import { LEVEL_MAP, PACMAN_START, GHOST_START, COLS, ROWS } from './level';
...
export const PacmanGame: React.FC<PacmanGameProps> = ({ onBack, audio, addCoins }) => {
    ...
    const ghostsRef = useRef<GhostType[]>([
        { id: 0, pos: { x: 9, y: 8 }, dir: 'LEFT', color: 'red', mode: 'CHASE', startPos: { x: 9, y: 8 }, speed: GAME_SPEED_BASE * 0.95 },
        { id: 1, pos: { x: 8, y: 10 }, dir: 'UP', color: 'pink', mode: 'CHASE', startPos: { x: 9, y: 10 }, speed: GAME_SPEED_BASE * 0.9 },
        { id: 2, pos: { x: 9, y: 10 }, dir: 'UP', color: 'cyan', mode: 'CHASE', startPos: { x: 9, y: 10 }, speed: GAME_SPEED_BASE * 0.85 },
        { id: 3, pos: { x: 10, y: 10 }, dir: 'UP', color: 'orange', mode: 'CHASE', startPos: { x: 9, y: 10 }, speed: GAME_SPEED_BASE * 0.8 }
    ]);
    ...
    const moveGhosts = () => {
        ghostsRef.current.forEach(g => {
            ...
            if (Math.abs(g.pos.x - xInt) < 0.1 && Math.abs(g.pos.y - yInt) < 0.1) {
                ...
                if (!isWall(xInt, yInt - 1) && g.dir !== 'DOWN') options.push('UP');
                if (!isWall(xInt, yInt + 1) && g.dir !== 'UP') options.push('DOWN');
                if (!isWall(xInt - 1, yInt) && g.dir !== 'RIGHT') options.push('LEFT');
                if (!isWall(xInt + 1, yInt) && g.dir !== 'LEFT') options.push('RIGHT');
    ...
    return (
        <div className="h-full w-full flex flex-col items-center bg-[#0a0a12] relative overflow-hidden text-white font-sans touch-none select-none">
            ...
            <div className="relative w-full max-w-md aspect-[19/21] bg-black border-2 border-blue-900/50 rounded-lg shadow-[0_0_20px_rgba(30,58,138,0.3)] overflow-hidden shrink-0">
                ...
                {/* OVERLAYS */}
                {!isPlaying && !gameOver && !gameWon && (
                    <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-20 backdrop-blur-sm" onClick={() => handleTouch(pacmanRef.current.dir)}>
                        <p className="text-white font-bold animate-pulse tracking-widest">APPUYEZ POUR JOUER</p>
                    </div>
                )}
            ...
            {/* CONTROLS (D-PAD) */}
            <div className="flex-1 flex items-center justify-center w-full max-w-md pb-6 pt-4">
                <div className="grid grid-cols-3 gap-2 w-48 h-48">
                    <div />
                    <button 
                        onPointerDown={(e) => { e.preventDefault(); handleTouch('UP'); }} 
                        className="bg-gray-800/80 rounded-xl flex items-center justify-center active:bg-yellow-500 active:text-black border border-white/10"
                    >
                        <ArrowUp size={32} />
                    </button>
                    <div />
                    ...
                    <div className="bg-black/40 rounded-full m-2 border border-white/5 flex items-center justify-center">
