...
import { Search, RefreshCw, X, UserPlus } from 'lucide-react';
...
export const CommunityView: React.FC<CommunityViewProps> = ({ 
    ...
    const onlineOnlyCommunity = React.useMemo(() => {
        return onlineUsers.filter(p => {
            if (p.name === currentUsername) return false;
    ...
    return (
        <div className="p-4 space-y-4 animate-in fade-in">
            ...
            <form onSubmit={handleSearchSubmit} className="flex gap-2">
                <div className="relative flex-1">
                    <input 
                        ...
                        className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:border-purple-500 outline-none pr-10" 
                    />
                ...
                <button type="submit" disabled={isSearching} className="bg-purple-600 text-white px-5 rounded-xl font-bold active:scale-95 transition-all shadow-lg hover:bg-purple-500">
            ...
            {searchResults.length > 0 ? (
                    <div className="space-y-2 animate-in fade-in slide-in-from-top-2">
                        ...
                        {searchResults.map(player => {
                            ...
                            return (
                                <div key={player.id} className="flex items-center justify-between p-3 bg-gray-800/80 rounded-2xl border border-purple-500/30 group cursor-pointer transition-all hover:bg-gray-700 shadow-lg" onClick={() => onPlayerClick(player)}>
            ...
            {!communitySearch && (
                <>
                    <h3 className="text-xs font-black text-gray-500 uppercase tracking-[0.2em] px-2 mt-2">Joueurs Actifs ({onlineOnlyCommunity.length})</h3>
                    <div className="space-y-2">
                        {onlineOnlyCommunity.length === 0 && <p className="text-center text-gray-500 text-xs italic py-4">Aucun autre joueur en ligne pour le moment.</p>}
