...
const BIOME_SWITCH_DISTANCE = 250; 
...
export const RunnerGame: React.FC<RunnerGameProps> = ({ onBack, audio, addCoins, onReportProgress }) => {
    ...
    const update = () => {
        ...
        const currentDistInt = Math.floor(distanceRef.current);
        ...
        if (missionRef.current && !missionRef.current.completed) {
            ...
            if (missionRef.current.type === 'DISTANCE') {
                progress = currentDistInt - missionRef.current.current;
            } else {
            ...
            if (progress >= missionRef.current.target) {
                ...
                const newMission = generateMission(
                    missionLevelRef.current, 
                    currentDistInt,
                    earnedCoins + missionRef.current.reward
                );
                missionRef.current = newMission;
                setCurrentMission(newMission);
                setMissionProgress(0);
        ...
        if (frameRef.current % 60 === 0 && onReportProgressRef.current) {
            onReportProgressRef.current('score', currentDistInt);
        }
    };
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => { if (e.code === 'Space' || e.code === 'ArrowUp') { e.preventDefault(); handleJump(e); } };
    ...
    return (
        <div className="h-full w-full flex flex-col items-center bg-black/20 relative overflow-hidden text-white font-sans touch-none select-none p-4" onMouseDown={handleJump} onTouchStart={handleJump}>
                            ...
                            {SKINS.map(skin => {
                                ...
                                return (
                                    <div key={skin.id} className={`p-3 rounded-xl border-2 flex flex-col items-center gap-2 transition-all ${isEquipped ? 'border-green-500 bg-green-900/20 shadow-[0_0_15px_rgba(34,197,94,0.3)]' : 'border-white/10 bg-gray-800/50'}`}>
                                        ...
                                        {isOwned ? (
                                            <button onClick={() => setCurrentSkinId(skin.id)} disabled={isEquipped} className={`w-full py-1.5 rounded-lg text-[10px] font-bold flex items-center justify-center gap-1 ${isEquipped ? 'bg-green-600 text-white cursor-default' : 'bg-gray-700 hover:bg-gray-600 text-gray-300'}`}>{isEquipped ? <><Check size={12}/> ÉQUIPÉ</> : 'CHOISIR'}</button>
                                        ) : (
                                            <button onClick={() => handleBuySkin(skin)} disabled={!canAfford} className={`w-full py-1.5 rounded-lg text-[10px] font-bold flex items-center justify-center gap-1 ${canAfford ? 'bg-yellow-500 hover:bg-yellow-400 text-black' : 'bg-gray-800 text-gray-500 cursor-not-allowed'}`}>{canAfford ? 'ACHETER' : <><Lock size={10}/> BLOQUÉ</>}</button>
                                        )}
                                    </div>
                                );
                            })}
            ...
            {gameOver && !showTutorial && (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/95 backdrop-blur-md z-50 animate-in zoom-in fade-in p-6">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); resetGame(); }} className="px-8 py-3 bg-orange-500 text-black font-black tracking-widest text-lg rounded-full hover:bg-white hover:scale-105 transition-all shadow-[0_0_20px_rgba(249,115,22,0.4)] flex items-center gap-2 pointer-events-auto"><RefreshCw size={20} /> REJOUER</button>
                </div>
            )}
        </div>
    );
