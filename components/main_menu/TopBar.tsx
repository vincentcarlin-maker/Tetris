...
export const TopBar: React.FC<TopBarProps> = ({ 
    ...
    return (
        <div className="absolute top-0 left-0 right-0 z-20 flex justify-between items-start p-4 pt-[calc(1.2rem+env(safe-area-inset-top))]">
            {isAuthenticated ? (
                featureFlags.economy_system ? (
                    <div ref={onCoinsRef} className="flex items-center gap-2 bg-black/80 backdrop-blur-md px-4 py-2 rounded-full border border-yellow-500/30 shadow-[0_0_15px_rgba(234,179,8,0.2)]">
                        ...
                        <span className="text-yellow-100 font-mono font-bold text-base">{coins.toLocaleString()}</span>
                    </div>
                ...
                <button onClick={onLoginRequest} className="flex items-center gap-2 bg-neon-blue/20 backdrop-blur-md px-4 py-2 rounded-full border border-neon-blue/50 hover:bg-neon-blue/40 transition-colors animate-pulse">
                    ...
                    <span className="text-neon-blue font-bold text-xs uppercase">{language === 'fr' ? 'SE CONNECTER' : 'LOGIN'}</span>
                </button>
            )}
            <div className="flex gap-2">
                ...
                <button onClick={onReload} className="p-2 bg-gray-900/80 rounded-full text-gray-400 hover:text-white border border-white/10 backdrop-blur-sm active:scale-95 transition-transform" title="Actualiser"><RefreshCw size={18} /></button>
            </div>
        </div>
    );
};
