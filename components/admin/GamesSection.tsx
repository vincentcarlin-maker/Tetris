...
import { Gamepad2, Edit2, Loader2 } from 'lucide-react';
...
export const GamesSection: React.FC<GamesSectionProps> = ({ disabledGames, setDisabledGames, mp }) => {
    ...
    const handleSaveConfig = (updatedGame: any) => {
        ...
        mp.sendAdminBroadcast(`${updatedGame.name} mis à jour`, 'info');
    ...
    const handleResetLeaderboard = async (gameId: string, gameName: string) => {
        ...
        const confirmReset = window.confirm(`⚠️ DANGER : RÉINITIALISATION ⚠️\n\nVous êtes sur le point de supprimer TOUS les scores pour : ${gameName}.\n\nCette action est irréversible pour tous les joueurs.\n\nÊtes-vous sûr de vouloir continuer ?`);
        ...
        try {
            ...
            setSelectedGame(null); // Fermer la modale après succès
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-in fade-in">
            {GAMES_CONFIG.map(game => {
                ...
                // Utilisation de l'icône définie dans GAMES_CONFIG ou Gamepad2 par défaut
                        ...
                        <div className="flex justify-between items-center">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] text-gray-500 font-mono uppercase">{game.category}</p>
                                </div>
                            </div>
            ...
            {selectedGame && (
                <GameConfigModal 
                    ...
                    onResetLeaderboard={() => handleResetLeaderboard(selectedGame.id, selectedGame.name)}
                    isResetting={resettingId === selectedGame.id}
                />
            )}
        </div>
    );
