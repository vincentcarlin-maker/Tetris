...
import { Search, ChevronRight, Coins, Clock, Shield, Ban, MicOff, UserCircle, X, Check, Loader2, Trash2 } from 'lucide-react';
        ...
        <div className="animate-in fade-in h-full flex flex-col">
            ...
            {selectedUser && (
                <UserDetailModal 
                    ...
                    onUpdate={(prop, val) => {
                        const updated = profiles.map(p => p.username === selectedUser.username ? { ...p, data: { ...p.data, [prop]: val } } : p);
                        setProfiles(updated);
                        setSelectedUser({ ...selectedUser, data: { ...selectedUser.data, [prop]: val } });
                        if (isSupabaseConfigured) DB.updateUserData(selectedUser.username, { [prop]: val });
                        mp.sendAdminBroadcast("Compte mis à jour", "user_update", { targetUser: selectedUser.username });
                    }}
                />
            )}
        </div>
...
const UserDetailModal = ({ user, onClose, onlineUsers, onUpdate }: any) => (
            ...
            <div className="p-6 space-y-6">
                <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gray-800 p-4 rounded-xl border border-white/10">
                        <p className="text-gray-500 text-xs font-bold uppercase mb-1">SOLDE PIÈCES</p>
                    ...
                    <div className="bg-gray-800 p-4 rounded-xl border border-white/10">
                        ...
                        <span className="text-lg font-bold text-white">{new Date(user.updated_at).toLocaleDateString()}</span>
                    </div>
                </div>
                <div className="flex flex-col gap-2 pt-4 border-t border-white/5">
                    ...
                    <button className="w-full py-3 bg-gray-800 text-gray-300 rounded-xl font-bold border border-white/5"><Trash2 size={18} className="inline mr-2"/> SUPPRIMER LE COMPTE</button>
                </div>
            </div>
        </div>
    </div>
