...
export const SnakeGame: React.FC<SnakeGameProps> = ({ onBack, audio, addCoins }) => {
    ...
    const handleGameOver = () => {
        ...
        // Calculate Rewards (1 coin per 10 points)
    ...
    const moveSnake = useCallback(() => {
        ...
        setSnake(prevSnake => {
            ...
            // Safety check against 180 turns based on current physical direction
            ...
            else setDirection(nextDirection); // Sync state
    ...
    useEffect(() => {
        if (isPlaying && !gameOver && !isPaused) {
            // Cancel previous loop to prevent speed stacking
    ...
    const handleDirectionChange = useCallback((newDir: Direction) => {
        ...
        setNextDirection(prevNext => {
            // Prevent changing to opposite of what's currently QUEUED (rapid key presses)
            const isOpposite = (newDir === 'UP' && prevNext === 'DOWN') ||
                               (newDir === 'DOWN' && prevNext === 'UP') ||
                               (newDir === 'LEFT' && prevNext === 'RIGHT') ||
                               (newDir === 'RIGHT' && prevNext === 'LEFT');
            
            // Also check against current actual direction to be safe
            const isOppositeCurrent = (newDir === 'UP' && direction === 'DOWN') ||
    ...
    const handleTouchMove = (e: React.TouchEvent) => {
        ...
        if (Math.abs(diffX) > threshold || Math.abs(diffY) > threshold) {
            ...
            touchStartRef.current = { x: touch.clientX, y: touch.clientY }; // Reset for continuous swipe
        }
    };
    ...
    const DPadBtn = ({ dir, icon: Icon }: { dir: Direction, icon: any }) => (
        <button 
            onMouseDown={(e) => { e.preventDefault(); handleDirectionChange(dir); }}
            onTouchStart={(e) => { e.preventDefault(); handleDirectionChange(dir); }}
            className="w-14 h-14 bg-gray-800/80 rounded-xl flex items-center justify-center border border-white/10 shadow-lg active:bg-green-500 active:text-black active:scale-95 transition-all text-green-400 group"
        >
            <Icon size={28} className="drop-shadow-[0_0_5px_currentColor] group-active:drop-shadow-none" />
        </button>
    );
    ...
    return (
        ...
        >
            ...
            <div className="relative w-full max-w-md aspect-square bg-black/80 border-2 border-green-500/30 rounded-xl shadow-[0_0_20px_rgba(34,197,94,0.2)] overflow-hidden backdrop-blur-md z-10">
                ...
                {snake.map((segment, i) => {
                    ...
                    return (
                        ...
                        >
                            <div className={`w-full h-full ${isHead ? 'bg-green-400 shadow-[0_0_10px_#4ade80]' : 'bg-green-600/80'} rounded-sm border border-black/20 transform scale-95`}>
                                {isHead && (
                                    <>
                ...
                {!isPlaying && !gameOver && !isPaused && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/60 backdrop-blur-sm z-30">
                        <p className="text-green-400 font-bold tracking-widest animate-pulse mb-4">APPUYEZ POUR JOUER</p>
                        <button onClick={startGame} className="w-16 h-16 rounded-full bg-green-500 flex items-center justify-center shadow-[0_0_20px_#22c55e] hover:scale-110 transition-transform">
                ...
                {/* Pause Overlay */}
                {isPaused && !gameOver && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 backdrop-blur-md z-30 animate-in fade-in">
                        <h2 className="text-4xl font-black text-white mb-6 tracking-widest">PAUSE</h2>
                        <div className="flex flex-col gap-3">
                            <button onClick={togglePause} className="px-8 py-3 bg-green-500 text-black font-bold rounded-full hover:bg-white transition-colors flex items-center justify-center gap-2">
                                <Play size={20} /> REPRENDRE
                            </button>
                            <button onClick={onBack} className="px-8 py-3 bg-gray-800 text-white font-bold rounded-full hover:bg-gray-700 transition-colors flex items-center justify-center gap-2">
                ...
                {/* Game Over Overlay */}
                {gameOver && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/90 backdrop-blur-md z-40 animate-in zoom-in fade-in">
                        ...
                        <button onClick={resetGame} className="px-8 py-3 bg-green-500 text-black font-black tracking-widest rounded-full hover:bg-white transition-colors shadow-lg flex items-center gap-2">
            ...
            {/* D-Pad for Mobile - FUNCTIONAL NOW */}
            <div className="w-full max-w-xs mt-6 grid grid-cols-3 gap-3 z-20">
                <div></div>
                ...
                <div className="flex justify-center"><div className="w-14 h-14 bg-gray-800/50 rounded-full flex items-center justify-center border border-white/5"><div className="w-4 h-4 bg-white/20 rounded-full"></div></div></div>
