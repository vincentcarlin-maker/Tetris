        ...
        <div className="h-full w-full flex flex-col items-center bg-black/20 relative overflow-y-auto text-white font-sans p-2">
            ...
            <div className="flex-1 w-full max-w-md flex flex-col gap-2 relative z-10 min-h-0 pb-2 justify-center">
                ...
                <div className="flex items-center justify-center gap-4 sm:gap-8 h-20 sm:h-32 relative shrink-0">
                    <div onClick={handleDeckClick} className={`relative cursor-pointer transition-transform ${turn === 'PLAYER' && !currentDrawnCard && (phase === 'PLAYING' || phase === 'LAST_TURN') ? 'hover:scale-105' : 'opacity-50'}`}><div className="w-10 h-14 sm:w-20 sm:h-28 bg-gray-800 rounded-md sm:rounded-lg border sm:border-2 border-purple-500 shadow-[0_0_15px_rgba(168,85,247,0.3)] flex items-center justify-center"><Layers className="text-purple-400" /></div><span className="absolute -bottom-5 w-full text-center text-[10px] font-bold text-gray-400">PIOCHE</span></div>
                    <div onClick={handleDiscardPileClick} className={`relative cursor-pointer transition-transform ${turn === 'PLAYER' && !currentDrawnCard && (phase === 'PLAYING' || phase === 'LAST_TURN') ? 'hover:scale-105' : ''}`}>{discardPile.length > 0 ? ( <SkyjoCardComponent card={discardPile[discardPile.length-1]} /> ) : ( <div className="w-10 h-14 sm:w-20 sm:h-28 bg-black/50 rounded-md sm:rounded-lg border sm:border-2 border-white/10 border-dashed flex items-center justify-center"></div> )}<span className="absolute -bottom-5 w-full text-center text-[10px] font-bold text-gray-400">DÉFAUSSE</span></div>
                    {currentDrawnCard && <div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20 flex flex-col items-center gap-2 animate-in zoom-in duration-200"><SkyjoCardComponent card={currentDrawnCard} highlight /><span className="text-[10px] font-bold bg-black/80 px-2 py-1 rounded text-white border border-white/20 shadow-lg">CARTE EN MAIN</span></div>}
                </div>
