...
export const ArenaUI: React.FC<ArenaUIProps> = ({
    ...
    const renderLobby = () => {
        ...
        return (
             <div className="absolute inset-0 z-[60] flex flex-col h-full bg-black/95 backdrop-blur-md items-center justify-center p-6 animate-in fade-in pointer-events-auto">
                 <div className="flex items-center gap-4 w-full max-w-md mb-8">
                    <button onClick={() => onSetGameState('MENU')} className="p-3 bg-gray-800 rounded-xl text-gray-400 hover:text-white border border-white/10 active:scale-95"><ChevronLeft size={24}/></button>
                    <h2 className="text-2xl font-black text-white italic">REJOINDRE L'ARÈNE</h2>
                 </div>
                 <div className="w-full max-w-md flex-1 flex flex-col gap-6 overflow-hidden">
                     <div className="bg-gradient-to-br from-red-900/40 to-black border border-red-500/30 rounded-3xl p-8 shadow-2xl relative overflow-hidden shrink-0 group">
                         <div className="absolute top-0 right-0 w-32 h-32 bg-red-500/10 rounded-full blur-2xl -mr-10 -mt-10"></div>
                         <h3 className="text-sm font-black text-white mb-6 flex items-center gap-2"><Wifi size={18} className="text-red-500"/> MODE HÔTE</h3>
                         <button onClick={mp.createRoom} className="w-full py-4 bg-red-600 hover:bg-red-500 text-white font-black tracking-[0.2em] rounded-2xl text-sm transition-all flex items-center justify-center gap-3 shadow-xl active:scale-95">
                            OUVRIR UN CANAL
                         </button>
                    </div>
                    <div className="flex-1 bg-black/40 backdrop-blur-md rounded-3xl border border-white/10 p-6 flex flex-col min-h-0">
                        <div className="flex justify-between items-center mb-6">
                            <span className="text-xs font-black text-gray-500 uppercase tracking-widest">Signaux détectés</span>
                            <span className="text-[10px] font-mono text-red-400 bg-red-900/30 px-3 py-1 rounded-full border border-red-500/30">{hostingPlayers.length} LIVE</span>
                        </div>
                        <div className="flex-1 overflow-y-auto space-y-4 custom-scrollbar pr-2">
                            {hostingPlayers.length > 0 ? hostingPlayers.map((player: any) => {
                                const avatar = avatarsCatalog.find(a => a.id === player.avatarId) || avatarsCatalog[0];
                                return (
                                     <div key={player.id} className="flex items-center justify-between p-4 bg-gray-800/40 hover:bg-gray-800 rounded-2xl border border-white/5 hover:border-red-500/30 transition-all group animate-in slide-in-from-right-4">
                                         <div className="flex items-center gap-4">
                                            ...
                                            <div><span className="font-black text-white group-hover:text-red-300 transition-colors block">{player.name}</span><span className="text-[9px] text-gray-500 font-bold uppercase tracking-widest">En attente...</span></div>
                                         </div>
                                         <button onClick={() => mp.joinRoom(player.id)} className="px-6 py-2.5 bg-white text-black font-black text-[10px] tracking-widest rounded-xl hover:bg-red-500 hover:text-white transition-all active:scale-95 shadow-lg">REJOINDRE</button>
                                     </div>
                                );
                            }) : (
                                <div className="flex-1 flex flex-col items-center justify-center text-gray-600 gap-6 opacity-30 h-full">
                                    <Search size={48} className="animate-pulse" />
                                    <p className="text-xs font-black tracking-[0.3em] text-center">SCAN DES FRÉQUENCES EN COURS...</p>
                                </div>
                            )}
                        </div>
                    </div>
    ...
    return (
        <div id="arena-ui-container" className="absolute inset-0 flex flex-col items-center pointer-events-none">
            {gameMode === 'ONLINE' && onlineStep !== 'game' && renderLobby()}
                ...
                <div className="absolute inset-0 z-50 flex flex-col items-center bg-[#020205] overflow-y-auto p-6 pointer-events-auto">
                    ...
                    <div className="relative z-10 w-full max-w-5xl flex flex-col items-center min-h-full justify-center">
                        <div className="mb-12 text-center animate-in slide-in-from-top-10 duration-700">
                             ...
                             <p className="text-red-400 font-black tracking-[0.5em] text-sm mt-4 uppercase">Le duel de lumière ultime</p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-4xl">
                            ...
                            <button onClick={() => { onSetGameMode('ONLINE'); onSetGameState('LOBBY'); }} className="group relative h-64 md:h-80 rounded-[40px] border border-white/10 bg-gray-900/40 backdrop-blur-md overflow-hidden transition-all hover:scale-[1.02] hover:border-orange-500/50 hover:shadow-[0_0_50px_rgba(249,115,22,0.3)] text-left p-8 flex flex-col justify-between">
                                ...
                                <div className="relative z-10"><div className="w-16 h-16 rounded-2xl bg-orange-500/20 flex items-center justify-center border border-orange-500/30 mb-6 shadow-xl"><Globe size={40} className="text-orange-400" /></div><div className="flex items-center gap-3 mb-2"><h2 className="text-4xl font-black text-white italic group-hover:text-orange-300">RESEAU LIVE</h2><span className="px-3 py-1 rounded-full bg-green-500/20 border border-green-500/50 text-green-400 text-[10px] font-black animate-pulse">EN LIGNE</span></div><p className="text-gray-400 text-sm font-medium leading-relaxed">Confrontation directe contre les cyber-pilotes mondiaux.</p></div>
            ...
            {gameState === 'DIFFICULTY' && (
                <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/95 backdrop-blur-md p-6 pointer-events-auto animate-in fade-in">
                        ...
                        {(Object.keys(ARENA_DIFFICULTY_SETTINGS) as Difficulty[]).map(d => {
                            ...
                            return (
                                <button key={d} onClick={() => onStartGame('SOLO', d)} className={`group flex items-center justify-between px-8 py-6 border-2 rounded-3xl transition-all ${s.color} hover:bg-gray-800 hover:scale-105 active:scale-95 shadow-xl`}>
                                    ...
                                    <div className="text-[10px] font-black font-mono text-right opacity-60 group-hover:opacity-100">RECOM x{s.coinMult}</div>
                                </button>
                            );
                        })}
                    </div>
                    <button onClick={onReturnToMenu} className="mt-12 text-gray-500 hover:text-white text-xs font-black tracking-[0.4em] uppercase border-b border-transparent hover:border-white transition-all">Annuler</button>
                </div>
            )}
            ...
            {(gameState === 'PLAYING' || gameState === 'RESPAWNING') && (
                <>
                    <div className="absolute top-0 left-0 w-full flex justify-between items-start p-6 z-20 pointer-events-none">
                        ...
                        <div className="w-40 bg-gray-900/90 p-4 rounded-3xl border border-white/10 backdrop-blur-md shadow-2xl">
                            <div className="flex items-center gap-2 mb-3 border-b border-white/10 pb-2"><Trophy size={16} className="text-yellow-400"/><span className="text-xs font-black text-white uppercase tracking-widest">Classement</span></div>
                            <div className="space-y-2">{leaderboard.slice(0, 5).map((p, i) => (<div key={i} className={`flex justify-between items-center text-[10px] ${p.isMe ? 'text-cyan-400 font-black animate-pulse' : 'text-gray-400 font-bold'}`}><span className="truncate w-24">{i+1}. {p.name.toUpperCase()}</span><span className="font-mono">{p.score}</span></div>))}</div>
                        </div>
                    </div>
                    <div className="absolute bottom-0 w-full h-56 grid grid-cols-2 gap-8 shrink-0 z-40 p-8 pointer-events-auto">
                        <div ref={leftZoneRef} className="relative bg-white/5 rounded-[40px] border border-white/10 flex items-center justify-center overflow-hidden active:bg-white/10 shadow-inner group">
                            <div className="absolute inset-0 flex items-center justify-center opacity-10"><div className="w-32 h-32 rounded-full border-4 border-cyan-500 animate-pulse"></div></div>
                            ...
                            <span className="absolute bottom-3 text-[9px] text-cyan-500 font-black tracking-[0.3em] uppercase">Propulsion</span>
                        </div>
                        <div ref={rightZoneRef} className="relative bg-white/5 rounded-[40px] border border-white/10 flex items-center justify-center overflow-hidden active:bg-white/10 shadow-inner group">
                            <div className="absolute inset-0 flex items-center justify-center opacity-10"><div className="w-32 h-32 rounded-full border-4 border-red-500 animate-pulse"></div></div>
                            ...
                            <span className="absolute bottom-3 text-[9px] text-red-500 font-black tracking-[0.3em] uppercase">Viseur Tactique</span>
                        </div>
                    </div>
                </>
            )}
            ...
            {gameState === 'GAMEOVER' && (
                <div className="absolute inset-0 z-[80] flex flex-col items-center justify-center bg-black/95 backdrop-blur-2xl animate-in zoom-in p-8 text-center pointer-events-auto">
                    ...
                    <div className="bg-gray-800/40 p-8 rounded-[40px] border border-white/10 mb-10 backdrop-blur-md shadow-2xl flex flex-col items-center">
                        <span className="text-xs font-black text-gray-500 uppercase tracking-[0.4em] mb-2">Eliminations confirmées</span>
                    ...
                    <div className="flex gap-6 w-full max-w-md">
                        <button onClick={() => { if(gameMode === 'ONLINE') onRematch(); else onStartGame(); }} className="flex-1 py-5 bg-red-600 text-white font-black tracking-[0.2em] rounded-3xl hover:bg-red-500 shadow-2xl flex items-center justify-center gap-3 active:scale-95 transition-all text-sm"><RefreshCw size={24} /> RE-SYNCHRONISER</button>
                        {gameMode === 'ONLINE' && <button onClick={onLeaveGame} className="flex-1 py-5 bg-gray-800 text-gray-300 font-black tracking-[0.2em] rounded-3xl hover:bg-gray-700 text-sm">QUITTER</button>}
                    </div>
                    {gameMode !== 'ONLINE' && <button onClick={onReturnToMenu} className="mt-10 text-gray-500 hover:text-white uppercase font-black text-xs tracking-[0.4em] border-b-2 border-transparent hover:border-white transition-all pb-1">Retour au menu principal</button>}
                </div>
            )}
        </div>
    );
